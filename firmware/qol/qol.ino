...
static constexpr uint32_t AUTO_DIM_IDLE_MS = 90000UL;
static constexpr uint32_t AUTO_SLEEP_IDLE_MS = 300000UL;
static constexpr uint32_t GO_SLEEP_ARM_DELAY_MS = 450UL;
static constexpr uint8_t AUTO_DIM_PERCENT = 30;
static constexpr uint8_t AUTO_DIM_MIN_BRIGHTNESS = 8;
static constexpr gpio_num_t GO_WAKE_GPIO = GPIO_NUM_0;
...
static bool autoDimActive = false;
static bool deepSleepPending = false;
static bool deepSleepReleaseSeen = false;
static uint32_t deepSleepPendingMs = 0;
static bool inputAnyDown = false;
...
static inline uint16_t addGlowToBuf(uint16_t baseBuf, RGB8 theme, uint8_t intensity, bool onDonut) {
    ...
    uint8_t rr = clampU8(r + ((int)theme.r * intensity) / div);
    uint8_t gg = clampU8(g + ((int)theme.g * intensity) / div);
    uint8_t bb = clampU8(b + ((int)theme.b * intensity) / div);
    return bufFromDraw(draw565(rr, gg, bb));
}
...
static void clearRenderBuffers() {
    resetDirtyRect();
    memset(zBuf + RENDER_START, 0, sizeof(uint16_t) * (size_t)RENDER_PIXELS);
    memset(shadeBuf + RENDER_START, 0, sizeof(uint8_t) * (size_t)RENDER_PIXELS);
    if (settings.glowEnabled) {
        memset(glowBuf + RENDER_START, 0, sizeof(uint8_t) * (size_t)RENDER_PIXELS);
    }
}
...
static void buildBackgroundCache(uint16_t* out) {
    memset(out, 0, sizeof(uint16_t) * (size_t)SCREEN_PIXELS);
...
static void copyBackgroundCacheToCanvas() {
    uint32_t t0 = micros();
    memcpy(canvasBuf + RENDER_START, bgCache + RENDER_START, sizeof(uint16_t) * (size_t)RENDER_PIXELS);
    bgCopyUs = micros() - t0;
}
...
static void drawBackgroundFxOverlay(uint16_t* out) {
    ...
    for (int i = 0; i < particles; ++i) {
        ...
        uint16_t h = (uint16_t)(iu * 1103u + (t >> 1));
        int x = (int)(((uint32_t)h * 37u + iu * 19u) % (uint32_t)SCREEN_W);
        int y = TOP_BAR_H + 8 + (int)(((uint32_t)h * 13u + iu * 29u) % (uint32_t)(RENDER_H - 12));
...
static void drawHelpPanel() {
    ...
    if (helpPage == 0) {
        ...
        printPanelLine(x + 5, y + 26, "Z Reset  GO Sleep R+~", drawPalette.hint);
    }
}
...
static uint8_t calcAutoDimBrightness() {
    uint16_t dim = ((uint16_t)settings.brightness * AUTO_DIM_PERCENT + 50u) / 100u;
    if (dim < AUTO_DIM_MIN_BRIGHTNESS) dim = AUTO_DIM_MIN_BRIGHTNESS;
    if (dim > settings.brightness) dim = settings.brightness;
    return (uint8_t)dim;
}

static void applyDisplayBrightness(uint8_t value) {
    if (appliedBrightness == value) return;
    M5Cardputer.Display.setBrightness(value);
    appliedBrightness = value;
}

static void restoreDisplayBrightness(uint32_t now) {
    autoDimActive = false;
    applyDisplayBrightness(settings.brightness);
    lastInputMs = now;
}

static void requestDeepSleep(uint32_t now, bool waitForGoRelease) {
    deepSleepPending = true;
    deepSleepReleaseSeen = !waitForGoRelease;
    deepSleepPendingMs = now;
    autoDimActive = false;
    applyDisplayBrightness(settings.brightness);
    pushToast(waitForGoRelease ? "GO SLEEP" : "AUTO SLEEP");
}

static void cancelDeepSleep(uint32_t now) {
    deepSleepPending = false;
    deepSleepReleaseSeen = false;
    applyDisplayBrightness(settings.brightness);
    lastInputMs = now;
}

static bool anyActionDown(const bool* down) {
    for (int i = 0; i < ACT_COUNT; ++i) {
        if (down[i]) return true;
    }
    return false;
}

static void enterDeepSleepNow() {
    if (prefsDirty) savePersistentSettingsNow();

    // Cardputer's G0 user button is GPIO0.  Wait for release before sleeping,
    // then use the next G0 press as the deep-sleep wake source.
    rtc_gpio_pullup_en(GO_WAKE_GPIO);
    rtc_gpio_pulldown_dis(GO_WAKE_GPIO);
    esp_err_t sleepErr = esp_sleep_enable_ext0_wakeup(GO_WAKE_GPIO, 0);
    if (sleepErr != ESP_OK) {
        deepSleepPending = false;
        deepSleepReleaseSeen = false;
        restoreDisplayBrightness(millis());
        pushToast("SLEEP ERR");
        return;
    }
...
static void serviceDisplayPower() {
    ...
    if (deepSleepPending) {
        if (deepSleepReleaseSeen && (uint32_t)(now - deepSleepPendingMs) >= GO_SLEEP_ARM_DELAY_MS) {
            enterDeepSleepNow();
        }
        return;
    }
    ...
    bool timerFlash = timerFlashUntilMs != 0 && (int32_t)(timerFlashUntilMs - now) > 0;
    bool watchBlocksAutoSleep = stopwatchState.running || stopwatchState.timerDone || timerFlash;
    if (idleMs >= AUTO_SLEEP_IDLE_MS && !watchBlocksAutoSleep) {
        requestDeepSleep(now, false);
        return;
    }

    if (idleMs >= AUTO_DIM_IDLE_MS) {
        if (!autoDimActive) {
            autoDimActive = true;
            applyDisplayBrightness(calcAutoDimBrightness());
        }
        return;
    }
...
static void handleInput() {
    ...
    bool persist = false;
    char msg[24];
    inputAnyDown = anyActionDown(down);
    bool goPressed = M5Cardputer.BtnA.wasPressed();
    bool goReleased = M5Cardputer.BtnA.wasReleased();

    if (deepSleepPending) {
        if (goReleased) {
            deepSleepReleaseSeen = true;
            deepSleepPendingMs = now;
        }
        if (inputAnyDown) {
            cancelDeepSleep(now);
            pushToast("SLEEP CANCEL");
        }
        for (int i = 0; i < ACT_COUNT; ++i) actionWasDown[i] = down[i];
        return;
    }

    if (autoDimActive && (inputAnyDown || goPressed)) {
        restoreDisplayBrightness(now);
        pushToast("BRIGHT ON");
        for (int i = 0; i < ACT_COUNT; ++i) actionWasDown[i] = down[i];
        return;
    }
